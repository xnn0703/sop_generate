---
name: doc-gen
description: 根据自然语言描述生成或修改作业指导书（SOP）。当用户提到"作业指导书 / 工艺指导书 / SOP / 生成产品手册 / 给某产品型号"等关键词时触发。底层调用 sop_generate 项目的 gen.py 渲染 HTML/PDF。
---

# Skill: doc-gen · 作业指导书生成

## 何时使用此 Skill

用户消息中出现以下任一关键字时优先调用本 Skill：
- 作业指导书 / 工艺指导书 / SOP / 工艺文件
- "给某型号 生成 / 修改 / 加一道工序"
- "新建产品手册"

## 项目位置

- 项目根：`/Users/xmac/Documents/project_manager/1-softHz/4-documents_generate/`
- Python 环境：项目下 `.venv/`（`.venv/bin/python`）
- 入口脚本：`gen.py`
- 产品 YAML：`products/<MODEL>.yaml`
- 图片目录：`assets/images/<MODEL>/`

## YAML schema（必须严格遵守）

```yaml
product:
  model: DEMO01                  # ASCII 字母数字/下划线/横线
  name: 演示产品                  # 推荐 ≤ 12 汉字
  company: 公司名称
  doc_id: SH-ZY-DEMO
  version: A/0                   # 形如 A/0、B/1
  publish_date: 2026-1-1         # YYYY-M-D
  effective_date: 2026-1-15

processes:                       # 1-32 道工序
  - name: 工序名                 # 推荐 ≤ 12 汉字
    key: false                   # true 表关键工序，会加 ★
    operations:                  # 1-18 条（单页 6 条，超 6 自动拆页）
      - 操作描述
    notes:                       # 0-4 条
      - 注意事项
    images:                      # 1-2 张，文件须存在于 assets/images/<MODEL>/
      - 工序名.png
    tools:                       # 1-4 项
      - 工具名
    materials:                   # 1-4 项
      - 材料名
```

**字段约束**：
| 字段 | 上限 | 处理 |
|------|------|------|
| processes 数 | 32 | 流程图按 8/页 自动分页 |
| operations 条数 | 18 | 单页 6 条，超 6 自动拆页（标"续 N/M"） |
| notes 条数 | 4 | 超过请拆工序（不会自动拆） |
| tools / materials 项数 | 4 各 | 超过请拆工序 |
| images 张数 | 1-4 | 1 张全占；2 张上下；3-4 张 2×2 自动缩放 |
| 字符长度 | 无硬限 | 超长自动换行；单元格 overflow:hidden 兜底，建议简洁表述 |

**重要提示**：当 operations 超过 6 条时虽然会自动拆页，但**优先考虑把工序拆成两道独立工序**——这样图片、工具、材料、注意事项也能各自配套，比续页更清晰。同理，图片建议每工序 1-2 张。

## 典型工作流

### 场景 1：新建产品

1. 询问/确认产品信息：型号、名称、工序清单
2. 复制 `products/DEMO01.yaml` 作为模板 → `products/<新型号>.yaml`
3. 创建图片目录 `assets/images/<新型号>/`，提示用户放入图片
4. 按用户描述填充 `processes`，**严格遵守字段约束**
5. 跑校验：`.venv/bin/python gen.py products/<新型号>.yaml --check`
6. 校验通过后：`.venv/bin/python gen.py products/<新型号>.yaml --pdf`
7. 报告：HTML / PDF 输出路径

### 场景 2：修改现有产品

1. 读 `products/<MODEL>.yaml` 理解现状
2. 按用户描述精确修改 YAML（用 Edit 工具，不要重写整个文件）
3. 跑校验 + 重新生成 PDF
4. 报告修改了哪些工序、哪些字段

### 场景 3：批量生成

`.venv/bin/python gen.py products/*.yaml --pdf`

## 关键约束

1. **图片必须真实存在**。如果用户描述的工序对应图片未放入 `assets/images/<MODEL>/`，**先提示用户放图片**，再继续 YAML 填充。
2. **不要在模板（templates/*.j2）里硬编码产品信息**。所有产品差异都通过 YAML 表达。
3. **超长字段要主动缩短**而不是机械填入。例如用户说"M4×10 内六角螺钉、螺纹胶"，应缩为"M4×10 螺钉、螺纹胶"或拆成两条。
4. **关键工序**（涉及静电敏感器件、精密装配等）的 `key` 字段设为 `true`。
5. **如果校验报错，停下来报告给用户**，不要继续生成 PDF。

## 常见错误处理

| 错误 | 处理 |
|------|------|
| `images 张数应为 1-2` | 提示用户提供图片，放到 `assets/images/<MODEL>/` |
| `operations 条数超过上限 18` | 拆成两道工序 |
| `notes / tools / materials 条数超过上限 4` | 拆成两道工序 |
| `图片不存在：...` | 检查文件名拼写，或提示用户补图 |
| PDF 导出失败 | 检查 Edge/Chrome 是否安装；可降级为仅生成 HTML，提示用户手动 ⌘P |

## 输出示例

完成后报告这样的信息：

```
✓ 已生成 DEMO02 作业指导书
  - YAML:  products/DEMO02.yaml（5 道工序，2 道为关键工序 ★）
  - HTML:  output/DEMO02.html
  - PDF:   output/DEMO02.pdf
  - 警告:  processes[2].notes[1] 偏长（29 > 28）— 建议缩短
```
